...
class AvatarConnectionInfo:
    room_name: str
    token: str
    participant_name: str = "AI Avatar"
    metadata: dict = None 
    participant_id: str = "avatar_worker"
...
class WorkerManager:
    ...
    def __init__(self):
        self.workers: dict[str, WorkerManager.WorkerInfo] = {}
    ...
    async def launch_worker(self, connection_info: AvatarConnectionInfo) -> None:
        ...
        cmd = [
            sys.executable,
            str(THIS_DIR / "avatar_worker_runner.py"),
            "--token", connection_info.token,
            "--room-id", connection_info.room_name,
            "--name", connection_info.participant_name,
            "--participant-id", connection_info.participant_id,
        ]

        try:
            room_name = connection_info.room_name
            process = subprocess.Popen(cmd, stdout=sys.stdout, stderr=sys.stderr)
            self.workers[room_name] = WorkerManager.WorkerInfo(

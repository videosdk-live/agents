...
from videosdk.agents.plugins import GoogleLLM, DeepgramSTT, CartesiaTTS
...
mongo_saver = MongoDBSaver(
    ...
    db_name="loan_assistant",
    collection_name="checkpoints",
)

config = GraphConfig(
    name="loan_assistant",
    graph_id="Loan_Assistant",
    debug=True,
    stream=True,
...
async def check_status_node(state: LoanState, ctx: Context):
    await ctx.say(
        ...
        "or call 1800-XXX-XXXX. Thankyou for calling. Have a nice day."
    )
    return END
...
class LoanAgent(Agent):
    ...
    async def on_enter(self) -> None:
        ...
        await self.session.say("Hello! I am your loan assistant. I can help you apply for a loan or check the status of your loan application.")
...
async def entrypoint(ctx: JobContext):
    ...
    await graph.compile(user_config={"user_id":"test_user01"})
    await graph.resume()
    graph.visualize()

...
async def entrypoint(ctx: JobContext):
    ...
    # Create unified pipeline with advanced EOU and Interrupt configurations
    pipeline = Pipeline(
        stt=DeepgramSTT(),
        llm=GoogleLLM(),
        tts=CartesiaTTS(),
    ...
    # Create session (no conversation_flow needed - pipeline handles it internally)
    session = AgentSession(agent=agent, pipeline=pipeline)

    await session.start(wait_for_participant=True, run_until_shutdown=True)

def make_context() -> JobContext:
    room_options = RoomOptions(room_id="<room_id>", name="Adavence Config Agent", playground=True)

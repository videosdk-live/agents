...
class RNNoise(Denoise):
    ...
    def _denoise_sync(self, audio_frames: bytes) -> bytes:
        ...
        if not self._logged_format:
            ...
            logger.info(
                "[RNNoise] input format detected: %d samples/frame "
                "(%d bytes) -> inferred %d Hz; resampling %s 48 kHz for the "
                "RNNoise model and back. Output is returned at %d Hz "
                "(same rate in == rate out, so VAD/STT are unaffected).",
                num_samples, len(audio_frames), original_sample_rate,
                "to/from" if original_sample_rate != self._target_sample_rate
                else "skipped at",

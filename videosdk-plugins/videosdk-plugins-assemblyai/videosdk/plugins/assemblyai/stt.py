...
class AssemblyAISTT(STT):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            language_code (str): The language code to use for the STT plugin. Defaults to "en_us".
            ...
            silence_threshold (float): The silence threshold to use for the STT plugin. Defaults to 0.015.
            silence_duration (float): The silence duration to use for the STT plugin. Defaults to 0.8.
        """
        super().__init__()
        ...
        if not self.api_key:
            raise ValueError(
                "AssemblyAI API key must be provided either through the 'api_key' parameter or the 'ASSEMBLYAI_API_KEY' environment variable.")

        self.language_code = language_code
        self.input_sample_rate = input_sample_rate
        self.target_sample_rate = target_sample_rate
        self.silence_threshold_bytes = int(silence_threshold * 32767)
        self.silence_duration_frames = int(
            silence_duration * self.input_sample_rate)
    ...
    async def _transcribe_buffer(self):
        ...
        try:
                    ...
                    if result["status"] == "completed":
                        if result.get("text") and self._transcript_callback:
                            event = STTResponse(
                                ...
                                data=SpeechData(text=result["text"], language=self.language_code, confidence=result.get(
                                    "confidence", 1.0))
                            )
            ...
            print(
                f"!!! ASSEMBLYAI PLUGIN FATAL ERROR: {e} ({type(e).__name__}) !!!")
    ...
    def _resample_audio(self, audio_bytes: bytes) -> bytes:
        ...
        resampled_data = signal.resample(mono_audio, int(
            len(mono_audio) * self.target_sample_rate / self.input_sample_rate))

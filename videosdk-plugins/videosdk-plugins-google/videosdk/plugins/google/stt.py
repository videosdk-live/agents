...
def _resolve_endpointing_sensitivity(value: Optional[str]):
    """Map a friendly string ('standard'/'short'/'supershort') to the proto
    EndpointingSensitivity enum. Returns None when unset or unrecognized — the
    caller then omits the field."""
    if value is None:
        return None
    enum = speech_types.StreamingRecognitionFeatures.EndpointingSensitivity
    key = value.strip().upper()
    if not key.startswith("ENDPOINTING_SENSITIVITY_"):
        key = f"ENDPOINTING_SENSITIVITY_{key}"
    try:
        return enum[key]
    except (KeyError, ValueError):
        logger.warning(
            "Unknown endpointing_sensitivity %r; expected one of "
            "'standard', 'short', 'supershort'. Ignoring.",
            value,
        )
        return None
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        Args:
            ...
            endpointing_sensitivity (Optional[str]): Server-side endpointing sensitivity controlling
                how quickly Google finalizes a result after speech ends. One of "standard" (default
                behavior, accuracy-biased), "short" (faster, for sentences/commands), or "supershort"
                (lowest latency, finalizes immediately on end of speech). Requires a model that
                supports it (e.g. "chirp_3"). The framework's flush() (gRPC half-close) already
                forces fast finalization, so this is a complementary/server-side backstop.
        ...
        resolved_endpointing = _resolve_endpointing_sensitivity(endpointing_sensitivity)
        ...
        self._config = {
            ...
            "endpointing_sensitivity": resolved_endpointing,
...
class SpeechStream:
    ...
    async def _audio_generator(self) -> AsyncGenerator[speech_types.StreamingRecognizeRequest, None]:
        ...
        try:
            ...
            endpointing_sensitivity = self._config.get("endpointing_sensitivity")
            streaming_features_kwargs = {
                ...
                "enable_voice_activity_events": (
                    ...
                    or endpointing_sensitivity is not None
                ),
                "voice_activity_timeout": voice_timeout_proto,
            }
            if endpointing_sensitivity is not None:
                streaming_features_kwargs["endpointing_sensitivity"] = endpointing_sensitivity

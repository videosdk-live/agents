...
def function_tool(func: Optional[Callable] = None, *, name: Optional[str] = None):
    ...
    def create_wrapper(fn: Callable) -> FunctionTool:
        ...
        tool_info = FunctionToolInfo(name=name or fn.__name__, description=fn.__doc__)
...
def build_openai_schema(function_tool: FunctionTool) -> dict[str, Any]:
    ...
    final_params_schema = (
        params_schema_to_use
        if params_schema_to_use is not None
...
def simplify_gemini_schema(schema: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if (
        result
        and result.get("type") == types.Type.OBJECT
...
def build_gemini_schema(function_tool: FunctionTool) -> types.FunctionDeclaration:
    ...
    if tool_info.parameters_schema is not None:
        if (
            tool_info.parameters_schema
            and tool_info.parameters_schema.get("properties", True) is not None
        ):
        ...
        if (
            openai_schema.get("parameters")
            and openai_schema["parameters"].get("properties", True) is not None
        ):
...
def create_generic_mcp_adapter(
    ...
    if not param_properties:
        ...
        async def param_tool(**kwargs) -> Any:
            ...
            if (
                "instructions" in required_params
                and "instructions" not in actual_kwargs
            ):
                other_params_provided = any(
                    p in actual_kwargs for p in param_properties if p != "instructions"
                )
                if other_params_provided:
                    actual_kwargs["instructions"] = (
                        f"Execute tool {tool_name} with the provided parameters."
                    )
...
def build_nova_sonic_schema(function_tool: FunctionTool) -> dict[str, Any]:
    ...
    return {
        "toolSpec": {
            ...
            "inputSchema": {"json": input_schema_json_string},
        }
    }

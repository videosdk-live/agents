...
class SpeechUnderstanding(EventEmitter[Literal["transcript_interim", "transcript_final", "speech_started", "speech_stopped", "eou_detected"]]):
    ...
    def __init__(
        ...
        # Fan-out queues: room recv loop enqueues here and returns immediately.
        # Consumers drain at their own pace so VAD/STT inference never blocks audio capture.
        self._vad_queue: asyncio.Queue[bytes | None] | None = None
        self._stt_queue: asyncio.Queue[bytes | None] | None = None
        self._vad_consumer_task: asyncio.Task | None = None
        self._stt_consumer_task: asyncio.Task | None = None
        self._vad_queue_max = 50      # ~1s of 20ms frames
        self._stt_queue_max = 100     # ~2s; STT tolerates more buffering
        self._vad_dropped = 0
        self._stt_dropped = 0
        self._consumers_started = False

        # EOU coalescing: cached probability reused across multiple STT FINALs within
        # the same user turn. Reset when the turn finalizes.
        self._last_eou_prob: float | None = None
    ...
    async def process_audio(self, audio_data: bytes) -> None:
        """
        Enqueue incoming audio for VAD and STT consumers.

        Returns quickly: denoise runs inline, then the chunk is enqueued onto
        per-consumer queues. The persistent consumer tasks drain them at their
        own pace, so VAD/STT inference never blocks the room recv loop.
    ...
    async def _run_stt_consumer(self) -> None:
        """Drain the STT queue, forwarding chunks to the configured STT instance.

        Acquires stt_lock around the provider call so a concurrent component swap
        waits for the in-flight chunk to complete before aclose()'ing the old STT.
        """
    ...
    async def _process_transcript_with_eou(self, new_transcript: str) -> None:
        ...
        Within a single user turn, multiple STT FINAL events can arrive (provider-
        dependent). The EOU probability is computed once on the first FINAL and
        reused for subsequent FINALs in the same turn; the wait timer still
        restarts on each FINAL so continued speech extends the window.
        ``_last_eou_prob`` is reset by ``_finalize_transcript`` when the turn
        truly ends, so the next user turn evaluates fresh.
        """
        async with self._transcript_processing_lock:
            ...
            if self.mode == 'DEFAULT':
                if self.turn_detector and self.agent:
                    eou_probability = await self._get_or_compute_eou_probability()
    ...
    async def _get_or_compute_eou_probability(self) -> float:
        """Return the cached EOU probability for this turn, or run inference once."""
        if self._last_eou_prob is not None:
            logger.info(f"EOU coalesced, reusing prob={self._last_eou_prob}")
            return self._last_eou_prob

        metrics_collector.on_eou_start()
        async with self.turn_detector_lock:
        ...
        self._last_eou_prob = eou_probability
